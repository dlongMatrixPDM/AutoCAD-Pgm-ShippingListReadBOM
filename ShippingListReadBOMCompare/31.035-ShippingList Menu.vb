            ...
            For z = 0 To BOMCnt                            '-------DJL-07-15-2025      'For Each DwgItem3 In VarSelArray
                ...
                With ReadBulkBOM
                    ...
                    If GenInfo3135.CustomerPO = Nothing Then                            '-------Need to add customer information to BOM so shipping list has everything required.
                        GenInfo3135.CustomerPO = .Range("AA3").Value
...
Err_FormatShipListRev0:
    ...
    Function FormatNewShipList(ByVal NewArray As Object, ByVal OldArray As Object, ByVal FileToFormat As Object, ByRef BOMSheet As String) As Object
        ...
        Dim Test, foundDeleted As String
        ...
        For iC = 1 To UBound(NewArray, 2)
            ...
            Select Case NewArray(0, iC)                            'Select Case NewArray(12, iC)
                Case "REVISED"
                    HighlightLine(iC + StrLineNo, "Y", BOMSheet)                            '2 Color number
                Case "NEW"
                    If NewArray(1, iC) <> vbNullString Then
        ...
        Select Case Comparison.InputType3.HeaderType        '-------Not sure this is doing anything-------DJL-------11-17-2023
            Case "True"
                For iC = 1 To UBound(NewArray, 2)
                    Test = NewArray(0, iC)                                     'Test = NewArray(11, iC)

                    If NewArray(0, iC) = "REVNO" Then                          'If NewArray(11, iC) = "REVNO" Then
        ...
        For iC = 1 To (UBound(OldArray, 2) - 1)                   '-----------------------------------Look for information that was deleted.
            If OldArray(0, iC) = vbNullString And OldArray(6, iC) <> vbNullString Then                            'If OldArray(UBound(OldArray, 1), iC) = vbNullString Then
                MultiLineMatl = False
                LineNo = LineNo + 1

                If foundDeleted = "Yes" Then
                    With NewShipListSht                     'ShipListSht
                        .Rows(LineNo & ":" & LineNo).Select()         '.Rows(RowNo & ":" & RowNo).Select()
                        .Rows(LineNo & ":" & LineNo).Insert()         '.Rows(RowNo & ":" & RowNo).Insert()
                    End With
                Else
                    With NewShipListSht
